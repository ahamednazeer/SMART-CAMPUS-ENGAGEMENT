    ...
    return (
        <DashboardLayout userRole="EMERGENCY" userName="Emergency Team" userEmail="emergency@airbase.mil">
            <div className="space-y-6">
                <h3 className="text-2xl font-chivo font-bold uppercase tracking-wider">Active Emergencies</h3>
                        ...
                        <table className="w-full">
                            <thead className="bg-slate-900/50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-mono text-slate-500 uppercase tracking-wider">Reported At</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-700/50">
                                {filteredEmergencies.map((emergency) => (
                                    <tr key={emergency.id} className="hover:bg-slate-800/50 transition-colors">
                                        <td className="px-6 py-4 font-mono text-sm text-red-400">{emergency.type}</td>
                                        <td className="px-6 py-4 text-sm text-slate-300">{emergency.description}</td>
                                        <td className="px-6 py-4 text-sm text-slate-300">{emergency.location}</td>
