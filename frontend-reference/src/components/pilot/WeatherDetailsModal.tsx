...
import { X, Cloud, Wind, Eye, Warning, Clock, ArrowUp, Phone, ArrowsClockwise } from '@phosphor-icons/react';
...
const runwayStatusConfig: Record<RunwayStatus | string, { label: string; color: string; bgColor: string }> = {
    OPEN: {
        label: 'RWY OPEN',
...
export default function WeatherDetailsModal({ weather, onClose, onRequestOpsAdvice, onRefresh }: WeatherDetailsModalProps) {
    ...
    const visMiles = weather.visibility
        ...
        : null;
    const rwyConfig = runwayStatusConfig[weather.runwayStatus || 'UNKNOWN'];
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-900 border border-slate-700 rounded-sm w-full max-w-md shadow-2xl relative overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-800/40">
                    <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-slate-500 text-xs uppercase tracking-wider font-mono">
                            Weather Details {weather.locationName ? `• ${weather.locationName}` : ''}
                        </h4>
                    </div>
                    <div className="flex items-center gap-2">
                        {onRefresh && (
                ...
                <div className="p-4 space-y-4 bg-slate-800/20">
                    ...
                    {weather.severeWeather && weather.severeWeather.length > 0 && (
                        <div className="flex items-center gap-2 px-2 py-1.5 bg-red-500/20 border border-red-500/40 rounded-sm">
                    ...
                    {/* Expanded Details (Always visible in modal) */}
                    <div className="space-y-2 pt-2 border-t border-slate-700/40">
                        ...
                        {/* Override info */}
                        {weather.isOverride && (
                            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-sm px-2 py-1.5 text-xs mt-2">
                    ...
                    {onRequestOpsAdvice && (
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-blue-600/20 border border-blue-500/40 text-blue-400 text-sm font-medium rounded-sm hover:bg-blue-600/30 transition-all mt-2"
                        >
                            <Phone size={14} />
                            Request Ops Advice
                        </button>
